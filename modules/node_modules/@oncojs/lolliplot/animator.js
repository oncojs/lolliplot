...
let animator: TAnimator = ({
  ...
  let draw = () => {
    ...
    data.proteins.forEach((d, i) => {
      ...
      d3Root.select(`.range-${d.id}-${d.start}-${d.end}`)
        .attrs({
      ...
      d3Root.select(`.clip-range-${d.id}-${d.start}-${d.end}-rect`)
        .attrs({
          x:  Math.max(yAxisOffset, x) + halfPixel,
          y: height - xAxisOffset + halfPixel,
          ...dim(Math.max(0, barWidth - 1), proteinHeight - halfPixel),
      ...
      d3Root.select(`.protein-name-${d.id}-${d.start}-${d.end}`)
        .attrs({
    ...
    d3Root.select(`.minimap-zoom-area`)
      .attrs({
        x: (min * scale) + yAxisOffset + halfPixel,
        width: minimapWidth,
      })

    d3Root.select(`.minimap-slide-target`)
      .attrs({
        x: (min * scale) + yAxisOffset + halfPixel + minimapWidth - 20,
      })

    d3Root.select(`.minimap-slide-target-arrow`)
      .attrs({
...
let animateScaleY: TAnimateScaleY = ({
  ...
  if (visibleMutations.length) {
    ...
    let numYTicks = d3Root.selectAll(`[class^='yTick-text']`).size()

    for (let i = 1; i < numYTicks + 1; i += 1) {
      d3Root.select(`.yTick-text-${i}`)
        .transition()
        .attr(`y`, scaleLinearY(i) + 3)

      d3Root.select(`.yTick-line-${i}`)
        .transition()
