...
let setupStats: TSetupStats = ({
  ...
  Object.keys(consequences).map(type => {
    consequencesContainer
      ...
      .html(`
        <span
          class="toggle-consequence-${type}"
          data-checked="true"
          style="color: ${consequenceColors[type]};
          text-align: center;
          border: 2px solid ${consequenceColors[type]};
      ...
      .on(`click`, () => {
        ...
        if (!d3.event.target.className.includes(`toggle-consequence`)) return

        let type = d3.event.target.className.split(`-`).pop()
        ...
        d3Root.select(`.toggle-consequence-${type}`)
          .html(checked ? `✓` : `&nbsp;`)
  ...
  Object.keys(impacts).map(type => {
    impactsContainer
      ...
      .html(`
        <span
          class="toggle-impacts-${type}"
          data-checked="true"
          style="
            color: ${impactsColors[type] || impactsColors.default};
            text-align: center;
            border: 2px solid ${impactsColors[type] || impactsColors.default};
            display: inline-block;
            width: 23px;
            cursor: pointer;
            margin-right: 6px;
      ...
      .on(`click`, () => {
        // Bail if not the checkbox above
        if (!d3.event.target.className.includes(`toggle-impacts`)) return
        ...
        d3Root.select(`.toggle-impacts-${type}`)
          .html(checked ? `✓` : `&nbsp;`)
...
let updateStats: TUpdateStats = ({
  ...
  Object.keys(consequences).map(type => {
    if (!visibleConsequences[type]) {
      d3Root.select(`.consquence-counts-${type}`)
  ...
  Object.keys(impacts).map(type => {
    if (!visibleImpacts[type]) {
      d3Root.select(`.impacts-counts-${type}`)
