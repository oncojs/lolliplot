...
let Lolliplot = ({
  ...
  svg
    ...
    .append(`circle`)
    .attrs({
      class: d => `mutation-circle-${d.id} ${d.id === highlightedPointId ? `selected-mutation` : ``}`,
      'clip-path': `url(#${uniqueSelector}-chart-clip)`,
      cx: d => scaleLinear(d.x),
      cy: d => scaleLinearY(d.y),
      r: d => (theme.mutationRadius + (collisions[`${d.x},${d.y}`] ? (collisions[`${d.x},${d.y}`].length > 5 ? 5 : collisions[`${d.x},${d.y}`].length) : 0)),
  ...
  svg
    ...
    .append(`text`).attrs({
      ...
      fontSize: `10px`,
      "text-anchor": `middle`,
    }).text(d => {return collisions[`${d.x},${d.y}`] ? collisions[`${d.x},${d.y}`].length : `` })
