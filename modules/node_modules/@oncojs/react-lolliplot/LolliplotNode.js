...
let Lolliplot = ({
  ...
  let base = Math.round(highestValue / numXTicks)
  data = data.map(d => ({ ...d, base, y: d.y + base }))

  let scaleLinearY = d3
    .scaleLinear()
    .domain([0, highestValue + base])
  ...
  svg
    ...
    .attrs({
      ...
      r: d => theme.mutationRadius + Math.min((collisions[`${d.x},${d.y - base}`] || []).length, 5),
  ...
  svg
    .append(`g`)
    .selectAll(`text`)
    .data(data.filter(d => collisions[`${d.x},${d.y - base}`]))
    .enter()
    .append(`text`)
    .attrs({
      x: d => scaleLinear(d.x),
    ...
    .text(d => collisions[`${d.x},${d.y - base}`].length)
  ...
  d3Root
    ...
    .append(`g`)
    .selectAll(`text`)
    .data(range(1 + base, highestValue + base, base))
    .enter()
    .append(`text`)
    .text(i => Math.round(i - base))
    .attrs({
